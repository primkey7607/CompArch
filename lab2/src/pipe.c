     ...
     switch (name) {
          ...
          case MUL:
             ...
             *result = op1 * op2;
             break;
          default:
             fprintf(stderr, "Problem with name of instruction\n");
             exit(1);
     }
     if (*result < 0){
        *N_flag = 1;
...
void exec_ADD() {
   ...
   //propagate these values to the next pipeline register
...
/* We need to propagate the flags for the next state */
   EX_MEM_REG.FLAG_Z = Z;
   EX_MEM_REG.FLAG_N = N;
   EX_MEM_REG.fff = 1;
}
...
void exec_B() {
   ...
   // properly squash the previous instructions
   memset(&DE_EX_REG, 0, sizeof(DE_EX_REG));
   memset(&IF_DE_REG, 0, sizeof(IF_DE_REG));
}
...
void pipe_stage_fetch()
{
  ...
  if (stall == 1){
     //just return here-do not change back to 0 yet
     return;
  }else {
     if (did_halt == 0){
