...
void wb_ADDS() {
   /*if (MEM_WB_REG.rd == 31){ //this is the zero register, so don't do anything
      CURRENT_STATE.REGS[MEM_WB_REG.rd] = 0; //X31 should always be 0
      return;
   }*/
   CURRENT_STATE.REGS[MEM_WB_REG.rd] = MEM_WB_REG.ALU_val;
}
...
void exec_CBZ() {
   ...
   if (forVal(DE_EX_REG.rn) == 0){
	  ...
	  if (DE_EX_REG.ptaken == 0){
         ...
         EX_MEM_REG.taken = 0;
         EX_MEM_REG.calc_PC = DE_EX_REG.PC_4; // CHECK commented out?
         //CURRENT_STATE.PC = CURRENT_STATE.PC - 4; // CHECK commented out?
      }
...
void set_format_ops(uint32_t instr)
{
  ...
  if (opc == 0){
     ...
     DE_EX_REG.ptaken = IF_DE_REG.ptaken;   // CHECK SHOULD THIS BE DONE IN ALL PLACES?? 
     return;
     ...
     if (did_halt == 0){
         if (squash == 1){
            // Will set squash to 0 in the execute stage
             ...
             // to predict, we need the current PC and a variable to store the result in
         } 
     }
  }  
}
