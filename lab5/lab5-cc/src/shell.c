...
void cycle() {                                                
  ...
  int cr;
  for (cr = 0; cr < NCOR; cr++) {
    if (active_core[cr]){  // if the corre is active
       crid = cr;
        pipe_cycle();
    }
  }
...
int done_sim() {
    ...
    for (cr = 0; cr < NCOR; cr++) {
        if (active_core[cr])    
           if (RUN_BIT[cr])
              simdone = 0;      // core cr is still running
    }
    return simdone;
}
/***************************************************************/
...
void rdump(FILE * dumpsim_file) {                               
  ...
  for (nc = 0; nc < NCOR; nc++) {
      ...
      //printf("MYPC                : 0x%" PRIx64 "\n", CURRENT_STATE[nc].PC);
      for (k = 0; k < ARM_REGS; k++) {
          printf("X%d: 0x%" PRIx64 "\n", k, CURRENT_STATE[nc].REGS[k]);
      }
      printf("FLAG_N: %d\n", CURRENT_STATE[nc].FLAG_N);
      printf("FLAG_Z: %d\n", CURRENT_STATE[nc].FLAG_Z);
  }
...
void get_command(FILE * dumpsim_file) {                         
  ...
  case 'i':
   if (scanf("%i %" PRIx64, &register_no, &register_value) != 2)
      break;
   CURRENT_STATE[0].REGS[register_no] = register_value;
   break;
...
void load_program(char *program_filename) {                   
  ...
  CURRENT_STATE[0].PC = MEM_TEXT_START;
